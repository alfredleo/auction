#!/bin/bash
## run 1.2 - run application [2022-07-15]
...
fallback_message=$"Usage: $0 { start | stop | init | test | seed | log-sql | remove-db | clear | load-env }, one or more commands separated by space"
if [[ $# -eq 0 ]]; then
...
runInit() {
    ...
    docker-compose exec php sh -c "chmod -R ugo+rw /var/www/html/storage"
...
echo " ## Script run for $runTime seconds ##"
